<section
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="relative bg-white rounded-lg shadow hover:shadow-md transition overflow-hidden">
            ...
            <div style="
                            ...
                            background-repeat: no-repeat;" class="h-48 flex items-center justify-center bg-gray-100 text-indigo-600 text-sm">
            ...
            <div style="
                            ...
                            background-position: top;
                            background-size: 100% 100%;
                            background-repeat: no-repeat;" class="h-48 flex items-center justify-center bg-gray-100 ">
                <div class="flex justify-center items-center bg-black bg-opacity-50 w-24 h-14 rounded-full">
    ...
    <!-- Modal Lightbox -->
    <div x-show="showModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" x-transition @click.self="showModal = false;
            ...
            <template x-if="['mp3','wav','ogg'].includes(mediaType)">
                <div class="bg-gray-900 rounded-xl p-5 shadow-lg flex flex-col items-center space-y-4 mt-4">
                    ...
                    <div class="flex items-center w-full gap-2">
                        ...
                        <input type="range" min="0" :max="duration" step="0.01" :value="currentTime" @input="$refs.file_audio.currentTime = $event.target.value; currentTime = $event.target.value;" class="flex-1 h-1 rounded-lg bg-gray-700 accent-green-500">
                        <span class="text-xs text-gray-400 w-8 text-left" x-text="formatTime(duration)">00:00</span>
                    </div>

                    <!-- Audio oculto -->
                    <audio x-ref="file_audio" :key="mediaSrc" class="hidden" @ended="ended = true; clearInterval(interval); currentTime = duration;">
